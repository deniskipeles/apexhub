...
export function DiscussionsPage() {
  ...
  useEffect(() => {
    if (currentRoute === 'discussion-detail' && routeParams.id) {
      setLoading(true);
      Promise.all([
        apex.collection('discussions').get(routeParams.id, { expand: 'posted_by_id' }).catch(() => null),
        apex.collection('discussions_conversations').list({
          filter: { discussion_id: Number(routeParams.id) || routeParams.id },
          sort: 'created',
          expand: 'author_id',
          per_page: 100
        }).catch(() => ({ items: [] }))
    ...
    return (
      <div className="p-6 md:p-12 max-w-5xl mx-auto">
        <RealtimeChat
          ...
          channel={`discussion_${routeParams.id}`}
        />
      </div>
    );
  }
