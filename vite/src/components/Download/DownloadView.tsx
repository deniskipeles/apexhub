...
import { Terminal, Monitor, Check, Copy, Download, ShieldCheck, Apple, Loader2 } from 'lucide-react';
...
export function DownloadView() {
    ...
    useEffect(() => {
        const fetchInfo = async () => {
            try {
                const apiUrl = import.meta.env.VITE_API_URL || 'https://api.apexkit.io';
        ...
        <div className="p-6 md:p-12 max-w-5xl mx-auto min-h-screen">
            ...
            <div className="max-w-3xl mx-auto mb-20">
                <div className="bg-surface border border-border rounded-2xl p-1.5 mb-6 flex justify-center w-fit mx-auto shadow-sm">
                    <button type="button" onClick={() => setSelectedOS('unix')} className={`px-6 py-2 rounded-xl text-sm font-medium transition-all ${selectedOS === 'unix' ? 'bg-background text-foreground shadow-sm ring-1 ring-border' : 'text-muted hover:text-foreground'}`}>Linux / macOS</button>
                    <button type="button" onClick={() => setSelectedOS('windows')} className={`px-6 py-2 rounded-xl text-sm font-medium transition-all ${selectedOS === 'windows' ? 'bg-background text-foreground shadow-sm ring-1 ring-border' : 'text-muted hover:text-foreground'}`}>Windows</button>
                </div>
                ...
                <div className="bg-zinc-950 border border-border rounded-2xl p-6 md:p-8 shadow-2xl relative group overflow-hidden">
                    <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button type="button" onClick={handleCopy} className="p-2 rounded-lg bg-white/10 hover:bg-white/20 text-zinc-400 hover:text-white transition-colors">
...
const DownloadCard = ({ os, arch, filename, icon, onDownload, isDownloading }: any) => (
    <div className="bg-surface border border-border rounded-xl p-6 hover:border-primary/50 transition-all group hover:shadow-lg duration-300">
        ...
        <button type="button" onClick={onDownload} disabled={isDownloading} className="w-full py-2.5 bg-background border border-border hover:bg-primary hover:text-white text-foreground font-medium rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50">
